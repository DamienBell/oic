		...
		if ($content->success()) { 
			$POD->addMessage("Content saved!");
                          if($_POST['type']== 'review'){
                                ...
                                //if a review is being added, then check to see if the user
                                //is already a member of the shop
                               //if not then add them
                                $shop_group= $POD->getGroup(array('id'=>$_POST['groupId']));
                                $shop_group->last_review= date('Y-m-d H:i:s');
                                $user= $POD->currentUser();
                                if(!$user->isMember($shop_group)){
                                    $shop_group->addMember($user);
                                }
                          }
