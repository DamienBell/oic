	...
	$(window).ready(function() {
		...
		$('a[href="#hideMessages"]').live('click',function(){
			...
			return false; 
                ...
                $('a[href="#toggleCommentBox"]').live('click', function(){
                    ...
                    if($(this).hasClass('active')){
                        ...
                        $(this).html($(this).attr('data-inactive'));
                        $('#comment_box'+id).hide();
                    }
                    else{
...
/* Display an error or alert */
	function complain(message,type) {
		...
		$('#system_messages').slideDown('fast',function() {
                    ...
                    $('#logo img').css('top', '0px');//gonzo specific
		});
		if (type=='success') {
	...
	function hideMessages() {
                ...
                $('#logo img').css('top', '30px');//gonzo specific
  ...
  function shorten(string, chars){
    if(string.length <= chars){
        return string;
         ...
         while((short_string.length +words[i].length)< (chars-3) && i < words.length){

            if(short_string.length + words[i].length== (chars- 3)){
                short_string += words[i];
            }else{
                  short_string += words[i]+" ";
                 }
            i++;
...
//paints each ratings box output by current.rating.php
function paintRatings(){
    $('.rating_box').each(function(){
         ...
         var atmos_avg= ($('input[name="atmos_avg'+id+'"]').val() * 10);
         var comm_avg=  ($('input[name="comm_avg'+id+'"]').val() * 10);
         var coff_avg=  ($('input[name="coff_avg'+id+'"]').val() * 10);
...
function startLoadWait(){
    $('#short_shop_description').append('<li id="loadblock"><h1>Loading</h1><h1>Awesome</h1> <h1>Shops</h1></li>');
}
